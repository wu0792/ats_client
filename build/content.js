...
const connContentAndBackground = chrome.runtime.connect({ name: _consts__WEBPACK_IMPORTED_MODULE_0__[/* CONNECT_ID_INIT_CONTENT */ "b"] })
...
function watchDomMutations() {
    mutationObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            ...
            if (targetSelector) {
                const message = {
                    action: _consts__WEBPACK_IMPORTED_MODULE_0__[/* ACTION_TYPES */ "a"].DOM_MUTATION.key,
                    type: mutation.type,
...
function doListenUserKeydown(ev) {
    ...
    if (targetSelector) {
        const message = { action: _consts__WEBPACK_IMPORTED_MODULE_0__[/* ACTION_TYPES */ "a"].USER_ACTIVITY_KEYDOWN.key, target: targetSelector, keyCode, ctrlKey, shiftKey, altKey }
        connContentAndBackground.postMessage(message)
        connContentAndPanel.postMessage(message)
    }
}
...
function doListenUserClick(ev) {
    ...
    if (targetSelector) {
        const message = { action: _consts__WEBPACK_IMPORTED_MODULE_0__[/* ACTION_TYPES */ "a"].USER_ACTIVITY_CLICK.key, target: targetSelector }
        connContentAndBackground.postMessage(message)
        connContentAndPanel.postMessage(message)
    }
}
...
function doListenUserScroll(ev) {
    if (lastScrollDate === null || (new Date() - lastScrollDate) >= commonThreshold) {
        ...
        const message = { action: _consts__WEBPACK_IMPORTED_MODULE_0__[/* ACTION_TYPES */ "a"].USER_ACTIVITY_SCROLL.key, scrollX, scrollY }
        connContentAndBackground.postMessage(message)
        connContentAndPanel.postMessage(message)
    }
}
...
function doListenUserResize(ev) {
    if (lastResizeDate === null || (new Date() - lastResizeDate) >= commonThreshold) {
        ...
        const message = { action: _consts__WEBPACK_IMPORTED_MODULE_0__[/* ACTION_TYPES */ "a"].USER_ACTIVITY_RESIZE.key, innerWidth, innerHeight }
        connContentAndBackground.postMessage(message)
        connContentAndPanel.postMessage(message)
    }
}
